...
isCmd ? addHit() : null
if (isCmd && m.isGroup) console.log(color('[', 'magenta'), color('➳', 'white'), color(']', 'magenta'), color('Comando'), color(`${prefix + command}`, 'white'), color('Nombre'), color(`${isOwner ? 'Es Mi creador 👑' : `${pushname}`}`, 'white'), color('En el Grupo'), color(`${groupMetadata.subject}`, 'yellow'), 'Args:', color(args.length))
if (isCmd && !m.isGroup) console.log(color('[', 'magenta'), color('➳', 'white'), color(']', 'magenta'), color('Comando'), color(`${prefix + command}`, 'white'), color('En Privado'), color('De', 'yellow'), color(`${isOwner ? 'Mi creador 👑' : `${pushname}`}`, 'white'), 'args:', color(args.length))
...
▢ *Límites restantes:* ${isOwner ? '∞' : isPre ? '∞' : `${limituser ? `${limituser - getLimit(m.sender)}` : limituser}`.replace('NaN', '100')}
...
*🎃 Script del bot:*
https://github.com/Tadashi669/HadesBot-MD`)
break
...
case 'reto': {
let { preguntas } = require('./lib/src/games/reto.js')
...
case 'verdad': {
let { preguntas } = require('./lib/src/games/verdad.js')
sendButMessage(preguntas[Math.floor(Math.random() * preguntas.length)],
buttons = [
{buttonId: prefix + 'reto', buttonText: {displayText: '‣ Reto'}, type: 1},
{buttonId: prefix + 'verdad', buttonText: {displayText: '‣ Verdad'}, type: 1},
...
case 'preguntas': {
let { preguntas } = require('./lib/src/games/pregunta.js')
sendButMessage(preguntas[Math.floor(Math.random() * preguntas.length)],
buttons = [
{buttonId: prefix + 'reto', buttonText: {displayText: '‣ Reto'}, type: 1},
{buttonId: prefix + 'verdad', buttonText: {displayText: '‣ Verdad'}, type: 1},
...
let { w, fail } = require('./lib/src/games/casino');
var deck = ['5', '5', '10', '5', '5']
